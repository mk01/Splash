...
static inline const char* select_error_str( int existing_error, const char* const existing_error_str, int new_error, const char* const new_error_str ) {
...
static void fb_setvt() {
	...
	const int   open_query_error     = ioctl( cur_tty, VT_OPENQRY, vtno);
